...
export interface LandingHeroProps {
  ...
  /** Accent blob color classes (3 blobs) */
  blobColors?: [string, string, string];
  /** Button accent: 'emerald' | 'blue' | 'violet' */
  accentColor?: 'emerald' | 'blue' | 'violet';
}

const BUTTON_STYLES: Record<string, string> = {
  emerald: 'bg-brand-emerald text-brand-slate shadow-[0_0_30px_rgba(110,231,183,0.2)] hover:shadow-[0_0_50px_rgba(110,231,183,0.3)]',
  blue: 'bg-brand-blue text-white shadow-[0_0_30px_rgba(96,165,250,0.2)] hover:shadow-[0_0_50px_rgba(96,165,250,0.3)]',
  violet: 'bg-brand-violet text-white shadow-[0_0_30px_rgba(167,139,250,0.2)] hover:shadow-[0_0_50px_rgba(167,139,250,0.3)]',
};
...
export function LandingHero({
  ...
  return (
    ...
    >
      ...
      {/* Layer 2: Atmospheric gradient blobs */}
      <div className="absolute inset-0 pointer-events-none">
        <motion.div
          className={`absolute top-[-10%] right-[-5%] w-[60%] h-[60%] ${blob1} blur-[150px] rounded-full`}
          animate={{ x: [0, 30, 0], y: [0, -20, 0] }}
          transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className={`absolute bottom-[-10%] left-[-5%] w-[45%] h-[45%] ${blob2} blur-[130px] rounded-full`}
          animate={{ x: [0, -20, 0], y: [0, 25, 0] }}
          transition={{ duration: 15, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className={`absolute top-[30%] left-[20%] w-[30%] h-[30%] ${blob3} blur-[120px] rounded-full`}
      ...
      >
        ...
        >
          ...
          {badge && (
            <motion.span
              variants={fadeUp}
              className={`inline-block text-[11px] font-semibold uppercase tracking-[0.25em] ${badgeColor} rounded-full px-5 py-2 mb-8 gradient-border-animated`}
            >
              {badge}
            </motion.span>
          )}
          ...
          >
            <span className="font-bold block">{headlineBold}</span>
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
            <Link href={primaryCta.href}>
              <Button
                size="lg"
                className={`h-14 px-8 text-base font-semibold hover:opacity-90 transition-all ${btnClass}`}
              >
                {primaryCta.label}
