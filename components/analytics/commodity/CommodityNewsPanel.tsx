...
const COMMODITY_KEYWORDS: Record<string, string[]> = {
  Gold: ['gold', 'bullion', 'precious metal', 'yellow metal', 'xau'],
  Silver: ['silver', 'precious metal', 'xag'],
  'Crude Oil': ['crude', 'oil', 'opec', 'brent', 'wti', 'petroleum'],
  'Natural Gas': ['natural gas', 'lng', 'gas prices'],
  Copper: ['copper', 'base metal', 'industrial metal'],
};

export function CommodityNewsPanel({ commodity }: Props) {
    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        if (res.success && res.data?.items) {
          const keywords = [
            ...(COMMODITY_KEYWORDS[commodity] || [commodity.toLowerCase()]),
            'commodity', 'mcx',
          ];
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      {news.length === 0 ? (
        <p className="text-xs text-muted-foreground">No {commodity.toLowerCase()}-related news found</p>
      ) : (
